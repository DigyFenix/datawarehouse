{#
  Mapa de llaves sustitutas de `cuenta`: (llave natural del ERP → entero autoincremental).
...
{{ config(
    ...
    unique_key = ['empresa_id', 'cuenta_codigo']
) }}

{{ mapa_llaves(ref('plata_cuenta'), ['empresa_id', 'cuenta_codigo']) }}
