...
| 4 | Agente (tools tipadas + 4 restricciones) | ✅ **construido 2026-08-08** — paquete `@pulso/agente` (dominio puro) + módulo en el portal de usuario. **4 tools tipadas** con SQL constante parametrizado, **las 4 restricciones con test cada una (12/12)**, tarjeta de dato armada del catálogo y no del texto del modelo, conversaciones persistidas y chat en el portal. Pendiente solo la `ANTHROPIC_API_KEY` para conversar contra el modelo | 2026-08-08 |
