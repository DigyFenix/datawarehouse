...
-- Tablas    : metadatos.catalogo_metricas
...
CREATE TABLE IF NOT EXISTS metadatos.catalogo_metricas (
  ...
  hecho_origen       text        NOT NULL
                       REFERENCES metadatos.catalogo_hechos (clave),
  filtros            jsonb       NOT NULL DEFAULT '{}'::jsonb,
  periodicidad       text,                               -- 'diaria' | 'mensual' | ...
  owner              text        NOT NULL,               -- Data Owner del dominio
  estado             metadatos.estado_metrica NOT NULL DEFAULT 'borrador',
...
COMMENT ON TABLE metadatos.catalogo_metricas IS
...
CREATE INDEX IF NOT EXISTS ix_metricas_estado ON metadatos.catalogo_metricas (estado);
CREATE INDEX IF NOT EXISTS ix_metricas_hecho  ON metadatos.catalogo_metricas (hecho_origen);
