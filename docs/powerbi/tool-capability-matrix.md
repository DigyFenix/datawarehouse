...
| **Ejecución DAX contra el modelo** | — | **No** | `which pbi-tools dax` → sin salida · `ls ~/.dotnet/tools` → sin salida · ningún MCP de Power BI conectado | **Gap crítico.** Candidato de la lista blanca: `microsoft/powerbi-modeling-mcp` (§3.5 — requiere autorización, no se instala) | F4/F6.2 quedan sin verificación directa |
